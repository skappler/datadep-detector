...
public class ClassLoaderHelper {
		...
		if (directory.exists() && directory.isDirectory()) {
			...
			for (final String file : files) {
				if (file.endsWith(".class")) {
					try {
						// System.out.println("Loading file "+pckgname +
						// "."+file);
	 ...
	 */
	private void checkJarFile(JarURLConnection connection, String pckgname, ArrayList<Class<?>> classes)
	...
	public ArrayList<Class<?>> getClassesForPackage(String pckgname) throws ClassNotFoundException {
		...
		try {
			...
			for (URL url = null; resources.hasMoreElements() && ((url = resources.nextElement()) != null);) {
				try {
					System.out.println("CrystalIT.ClassLoaderHelper.getClassesForPackage() Checking " + url);
					...
					System.out.println("ClassLoaderHelper.getClassesForPackage() Ignored" + e.getMessage());
				}
			}
